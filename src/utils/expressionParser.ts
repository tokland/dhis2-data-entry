...
export class ExpressionParser {
    ...
    private static ifExp = /if\((?<condition>[^,]+),(?<valueT>[^,]+),(?<valueF>[^)]+)\)/;
    ...
    private static detectExpression(type: ExpressionType, expression: string): Expression | null {
        ...
        if (expression.match(this.ifExp)) {
            const match = expression.match(this.ifExp);
            if (!match) return null;

            const ifExpr = match.groups as { condition: string; valueT: string; valueF: string };
            const expressions$ = [
                this.parse(type, ifExpr.condition),
                this.parse(type, ifExpr.valueT),
                this.parse(type, ifExpr.valueF),
            ];

            return Either.join(expressions$).match({
                success: ([condition = [], valueTrue = [], valueFalse = []]) => {
                    return { type: "if", condition, valueTrue, valueFalse };
                },
                error: () => null,
            });
        }
